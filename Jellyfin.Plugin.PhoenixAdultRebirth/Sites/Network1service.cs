        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            switch (domain)
            {
                case "www.brazzers.com":
                    if (sceneTypeURL.Equals("serie", StringComparison.OrdinalIgnoreCase) ||
                        sceneTypeURL.Equals("scene", StringComparison.OrdinalIgnoreCase))
                    {
                        sceneTypeURL = "video";
                    }
            ...
            foreach (var actorLink in sceneData["actors"])
            {
                ...
                if (actorData != null)
                {
                    ...
                    var actor = new PersonInfo { Name = (string)actorLink["name"] };
                    ...
                    foreach (JProperty image in sceneData["images"][imageType])
                    {
                        if (int.TryParse(image.Name, out _))
                        {
                            result.Add(
                                new RemoteImageInfo
                                {
                                    Url = (string)sceneData["images"][imageType][image.Name]["xx"]["url"], Type = ImageType.Primary,
                                });
                            result.Add(
                                new RemoteImageInfo
                                {
                                    Url = (string)sceneData["images"][imageType][image.Name]["xx"]["url"], Type = ImageType.Backdrop,
                                });
                        }
                    }
                }
