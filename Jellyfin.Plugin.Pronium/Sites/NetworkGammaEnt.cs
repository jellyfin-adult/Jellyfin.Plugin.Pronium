    ...
    public class NetworkGammaEnt : IProviderBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var sceneType in sceneTypes)
            {
                ...
                {
                    string sceneID, sceneName = (string)searchResult["title"], curID;
                    ...
                    var res = new RemoteSearchResult { Name = sceneName };
                    ...
                    res.ProviderIds.Add(Plugin.Instance?.Name ?? "Pronium", curID);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            string domain = new Uri(Helper.GetSearchBaseURL(siteNum)).Host, sceneTypeURL = sceneID[1] == "scenes" ? "video" : "movie";
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var siteList = new List<string> { "girlsway.com", "puretaboo.com" };
            ...
            if (!ignore)
            {
                image = (sceneData.ContainsKey("url_movie_title") ? sceneData["url_movie_title"] : sceneData["url_title"]).ToString()
                    .ToLowerInvariant().Replace('-', '_');
                imageURL =
                    $"https://images-fame.gammacdn.com/movies/{sceneData["movie_id"]}/{sceneData["movie_id"]}_{image}_front_400x625.jpg";

                result.Add(new RemoteImageInfo { Url = imageURL, Type = ImageType.Primary });
            }
            ...
            if (sceneData.ContainsKey("pictures"))
            {
                ...
                result.Add(new RemoteImageInfo { Url = imageURL, Type = ImageType.Primary });
                result.Add(new RemoteImageInfo { Url = imageURL, Type = ImageType.Backdrop });
            }
