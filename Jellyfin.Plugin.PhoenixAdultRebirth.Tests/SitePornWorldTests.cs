...
public class SitePornWorldTests
{
    ...
    private readonly string _testSceneRegularUrl = "https://pornworld.com/watch/465350/insatiable_babe_rebecca_volpetti_gets_dp_d_during_boxing_training_gp2727";
    ...
    {
        var result = await _site.Search(new []{52, 0}, "Insatiable Babe Rebecca Volpetti Gets DP'D During Boxing Training", null, new CancellationToken());
        Assert.That(result.Count, Is.GreaterThanOrEqualTo(21));
        var id = result[0].ProviderIds.Values.FirstOrDefault();
        Assert.That(id, Is.Not.Empty);
        Assert.That(Helpers.Helper.Decode(id?.Split('#')[0]), Is.EqualTo(_testSceneRegularUrl));
    }

    [Test]
    public async Task SearchByIdIsWorking()
    {
        var result = await _site.Search(new []{52, 0}, "Valentine's Day Threesome With Lesbo Lover Shalina Devine and Rebecca Volpetti GP2582", null, new CancellationToken());
        ...
        Assert.That(Helpers.Helper.Decode(id?.Split('#')[0]), Is.EqualTo(_testSceneByIdUrl));
    }

    [Test]
    public async Task UpdateIsWorking()
    {
        var result = await _site.Update(new []{52, 0}, new []{Helpers.Helper.Encode(_testSceneRegularUrl), "2010-09-14"}, new CancellationToken());
    ...
    {
        var result = (await _site.GetImages(new[] { 52, 0 }, new[] { Helpers.Helper.Encode(_testSceneRegularUrl), "2010-09-14" }, null, new CancellationToken())).ToList();
