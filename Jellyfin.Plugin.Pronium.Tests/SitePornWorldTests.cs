...
public class SitePornWorldTests
{
    ...
    {
        var result = await _site.Search(new[] { 52, 0 }, "Insatiable Babe Rebecca Volpetti Gets DP'D During Boxing Training", null,
    ...
    {
        var result = await _site.Search(new[] { 52, 0 },
            "Valentine's Day Threesome With Lesbo Lover Shalina Devine and Rebecca Volpetti GP2582", null, new CancellationToken());
    ...
    {
        var result = await _site.Update(new[] { 52, 0 }, new[] { Helper.Encode(_testSceneRegularUrl), "2010-09-14" },
            new CancellationToken());
        ...
        Assert.That(result.People.First().ImageUrl,
            Does.StartWith(
    ...
    {
        var result = (await _site.GetImages(new[] { 52, 0 }, new[] { Helper.Encode(_testSceneRegularUrl), "2010-09-14" }, null,
            new CancellationToken())).ToList();

        Assert.That(result, Has.Count.EqualTo(2));
        Assert.That(result.First().Url,
            Is.EqualTo(
