...
public class SitePrivateTests
{
    ...
    public async Task SearchForMovieIsWorking()
    {
        var result = await _site.Search(
            new[] { 53, 0 },
            "The Bachelorette Bang",
            DateTime.Parse("10/01/2011", CultureInfo.InvariantCulture),
    ...
    public async Task SearchForSceneIsWorking()
    {
        var result = await _site.Search(
            ...
            DateTime.Parse("05/06/2012", CultureInfo.InvariantCulture),
            new CancellationToken());
        Assert.That(result.Count, Is.EqualTo(1));
        var id = result[0].ProviderIds.Values.FirstOrDefault();
        Assert.That(id, Is.Not.Empty);
    ...
    public async Task GetImagesForMovieIsWorking()
    {
        var result = (await _site.GetImages(
            new[] { 53, 0 },
            new[] { Helper.Encode(_testMovieUrl), "2010-09-14" },
            null,
            new CancellationToken())).ToList();
